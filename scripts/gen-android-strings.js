...
/*
 * gen-android-strings.js — produce a single, dense UI-string table for the Android app.
 *
 * Identical merge to gen-ios-strings.js (the web app resolves a UI string at runtime via
 * i18n.t(lang, key)):
 ...
 *
 * The Android app has no JS runtime, so this pre-resolves that exact merge for EVERY
 * canonical language and writes one flat JSON the app loads from its assets:
 *
 *     android/app/src/main/assets/strings.json   { "<lang>": { "<key>": "<value>" }, ... }
 *     android/app/src/main/assets/languages.json  (verbatim copy of data/languages.json)
 *
...
const OUT_DIR = path.join(ROOT, 'android', 'app', 'src', 'main', 'assets');
