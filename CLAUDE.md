...
- **Script load order matters** (set in `index.html`): `countries.js` → `i18n.js` → `vendor/leaflet/leaflet.js` → `app.js`.
...
- **i18n.js** — UI strings for `en/es/ar/fr/uk` with English fallback, plus `detectLanguage`/`isRTL`/`buildInitialMessage`. Add any new UI string to **all five** language objects (English is canonical).
